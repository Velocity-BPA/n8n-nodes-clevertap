    ...
    try {
      ...
      switch (operation) {
        case 'sendPushMessage': {
          ...
          try {
            parsedPlatformSpecific = typeof platformSpecific === 'string' ? JSON.parse(platformSpecific) : platformSpecific;
          } catch (error: any) {
            throw new NodeOperationError(this.getNode(), `Invalid platform_specific JSON: ${error.message}`);
          }

          const body = {
            to: to,
            notification: parsedNotification,
            platform_specific: parsedPlatformSpecific,
          };
        ...
        case 'createMessageTarget': {
          const targetName = this.getNodeParameter('target_name', i) as string;
          const query = this.getNodeParameter('query', i) as any;
